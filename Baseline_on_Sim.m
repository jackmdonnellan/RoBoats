...
%% Tracker Code
tic
bg = BackgroundImage('Double_Gyre_Sim/Image*.jpg','background.jpg');
figure; imagesc(bg); colormap gray; title('background image');
[vtracks,ntr,lm,lrms] = ...
    PredictiveTracker('Double_Gyre_Sim/Image*.jpg',15,10,'background.jpg',3,0,0);

%% Velocity Information
[u,v,x,y,t]=Velocities(vtracks,[3 inf],0);
...
interpPoints = combvec(interpX,interpY)';
...
for ii = 1:ct(end,1)
    if ct(ii,2) >= 40 %if there are more than 40 particles in the frame
        count = count+1;
        if mod(count,5) == 0 %get data about three a second
